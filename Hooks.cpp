...
void __fastcall PaintTraverse_Hooked(PVOID pPanels, int edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	else if (FocusOverlayPanel == vguiPanel)
	{
		...
		Render::Text(10, 10, Color::FromHSB(rainbow, 1.f, 1.f), Render::Fonts::Menu, "");
		...
		if (Delay) {
			if (GetTickCount() - Delay > 850) {
				...
				if (Interfaces::Engine->GetPlayerInfo(Interfaces::Engine->GetLocalPlayer(), &PlayerInfo)) {
					char Buffer[128];
					sprintf(Buffer, "say [NeonCheats] %s - %i kill(s)", PlayerInfo.name, Kills2);
...
bool __fastcall Hooked_FireEventClientSide(PVOID ECX, PVOID EDX, IGameEvent *Event) {
	if (Menu::Window.MiscTab.OtherEventSpam.GetState()) {
		if (strcmp(Event->GetName(), "round_end") == 0) {
			...
			char Buffer[128];
			sprintf(Buffer, "say NeonCheats.uk.to");
			Interfaces::Engine->ClientCmd(Buffer);
		}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo)) {
				char Buffer[128];
				sprintf(Buffer, "say NeonCheats - %s just connected", PlayerInfo.name);
				Interfaces::Engine->ClientCmd(Buffer);
			}
		}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo) &&
				...
				if (PlayerID != Interfaces::Engine->GetLocalPlayer() && AttackerID == Interfaces::Engine->GetLocalPlayer()) {
					...
					switch (Kills) {
					case 1: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 2:	sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 3:	sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 4:	sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 5:	sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 6:	sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 7:	sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 9: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 15: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 21: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 26: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 37: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					case 40: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name); break;
					default: sprintf(Buffer, "say NeonCheats - %s just got fucked!", PlayerInfo.name, Kills); break;
					}
			...
			if (Interfaces::Engine->GetPlayerInfo(PlayerID, &PlayerInfo)) {
				char Buffer[128];
				sprintf(Buffer, "say NeonCheats - %s just rage quit :)", PlayerInfo.name);
				Interfaces::Engine->ClientCmd(Buffer);
			}
		}
	}
